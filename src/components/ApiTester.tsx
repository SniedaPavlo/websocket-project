...
export const ApiTester: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (hasRunRef.current) {
      console.log("🔄 ApiTester: Already executed, skipping...");
      return;
    }

    hasRunRef.current = true;
    console.log("🚀 ApiTester: Starting initial execution...");
    ...
    const testAPI = async () => {
      ...
      try {
        ...
        if (competitions.length > 0) {
          ...
          console.log(
            "  🕐 Start Time:",
            new Date(first.startTime * 1000).toISOString()
          );
          console.log(
            "  🕐 End Time:",
            new Date(first.endTime * 1000).toISOString()
          );

          console.log("\n📋 All competition keys:");
          competitions.forEach((comp, index) => {
            console.log(
              `  ${index + 1}. ${comp.competitionKey} (${comp.priceFeedId})`
            );
          });
        }
      } catch (error) {
      ...
      try {
        ...
        if (activeCompetitions.length > 0) {
          ...
          activeCompetitions.forEach((comp, index) => {
            ...
            console.log(
              `      End time: ${new Date(comp.endTime * 1000).toISOString()}`
            );
            console.log(
              `      Time left: ${timeLeft} seconds (${Math.round(
                timeLeft / 3600
              )} hours)`
            );
          });
      ...
      try {
        ...
        if (solCompetition) {
          ...
          console.log(
            "  🕐 Start:",
            new Date(solCompetition.startTime * 1000).toISOString()
          );
          console.log(
            "  🕐 End:",
            new Date(solCompetition.endTime * 1000).toISOString()
          );
